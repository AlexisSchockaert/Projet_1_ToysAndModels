...
SELECT MONTH(orderdate) as month_,Productcode, Productname,ProductLine, MSRP,round(avg(priceeach),2) as Vendu_en_moyenne, 
sum(quantityordered) as Sum_qté_vendu, 
sum(quantityordered*MSRP) as Total_MSRP, 
sum(quantityordered*priceEach) as Total_Vendu
FROM orderdetails
INNER JOIN products
USING (productcode)
INNER JOIN orders
...
GROUP BY month_,Productcode, Productname,ProductLine, MSRP);

SELECT *, Total_vendu - Total_MSRP as Diff
From Pave
ORDER BY Diff ASC
;
