WITH best_sellors AS( 
    SELECT  YEAR(orders.orderDate) as year_,MONTH(orders.orderDate) as Month_,concat(employees.firstname," ",employees.lastname) as sellor, offices.country,  
    sum(orderdetails.priceEach*orderdetails.quantityOrdered) AS turnover, 
    RANK() OVER (PARTITION BY YEAR(orders.orderDate),MONTH(orders.orderDate) ORDER BY sum(orderdetails.priceEach*orderdetails.quantityOrdered) desc) as rank_best_sellor 
    FROM orderdetails 
    JOIN orders ON orders.orderNumber=orderdetails.orderNumber 
    JOIN customers ON customers.customerNumber = orders.customerNumber 
    JOIN employees ON employees.employeeNumber = customers.salesRepEmployeeNumber 
    JOIN offices ON offices.officeCode = employees.officeCode 
    WHERE YEAR(orders.orderDate)>YEAR(NOW())-1 and MONTH(orders.orderDate)>MONTH(NOW())-3
